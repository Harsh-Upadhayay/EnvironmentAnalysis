...
def stateBox(placeHolder, firstMessage, secondMessage, firstPos = 0):
    try :    
        ...
        l2 = Label(f1, text=secondMessage).grid(
            column=0, row=1, padx=8, pady=4)

        e2 = Entry(f1, textvariable=stateName).grid(
            column=1, row=1, padx=8, pady=4)

        b1 = Button(f1, text="show", command=lambda : WBD_TA_State(placeHolder, countryName, stateName)).grid(
...
def countryBox(placeHolder, firstMessage, secondMessage, firstPos = 0):
    try :    
        ...
        b1 = Button(f1, text="show", command=lambda : WBD_TA_Country(placeHolder, countryName)).grid(
