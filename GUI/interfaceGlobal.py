...
def countryBox(placeHolder, firstMessage, secondMessage, firstPos=0):
    try:
        ...
        b1 = tkinter.Button(f1, text="show using Pandas", command=lambda: kaggle_TA_Country(placeHolder, countryName, month, s_year, e_year)).grid(
...
def stateBox(placeHolder, firstMessage, secondMessage, firstPos=0):
    try:
        ...
        s2_2 = tkinter.Scale(f1, from_=1854, to=2010, orient=tkinter.HORIZONTAL,
                             variable=e_year).grid(row=3, column=1)
        b1 = tkinter.Button(f1, text="show", command=lambda: kaggle_TA_State(placeHolder, stateName, month, s_year, e_year)).grid(
...
def cityBox(placeHolder, firstMessage, secondMessage, firstPos=0):
    try:
        ...
        s2_2 = tkinter.Scale(f1, from_=1854, to=2010, orient=tkinter.HORIZONTAL,
                             variable=e_year).grid(row=3, column=1)
        b1 = tkinter.Button(f1, text="show using Pandas", command=lambda: kaggle_TA_Cities(placeHolder, cityName, month, s_year, e_year)).grid(
            column=0, row=4, columnspan=2, pady=2)
