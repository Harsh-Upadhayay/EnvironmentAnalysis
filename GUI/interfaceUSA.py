...
def cityBox(placeHolder, firstMessage, secondMessage, firstPos = 0):
    try :    
        ...
        l2 = tkinter.Label(f1, text="Enter month (Leave blank to take all data)",bg='lightpink2',font=("dongle", 10)).grid(
            column=0, row=1, padx=8, pady=4)

        e2 = tkinter.Entry(f1, textvariable=month).grid(
            column=1, row=1, padx=8, pady=4)

        l3 = tkinter.Label(f1, text="Enter day (Leave blank to take all data)",bg='lightpink2',font=("dongle", 10)).grid(
            column=0, row=2, padx=8, pady=4)

        e3 = tkinter.Entry(f1, textvariable=day).grid(
            column=1, row=2, padx=8, pady=4)

        b1 = tkinter.Button(f1, text="show", command=lambda : USA_TA_City(placeHolder, cities, month, day),bg='goldenrod1').grid(
